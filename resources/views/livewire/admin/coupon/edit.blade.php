<div class="main-content-wrap">
    ...
    <div class="wg-box">
        <form class="form-new-product form-style-1" method="POST"  wire:submit.prevent="updateCoupon()">
            @csrf
            <fieldset class="name">
                <div class="body-title">Coupon Code <span class="tf-color-1">*</span></div>
                <input class="flex-grow" type="text" placeholder="Coupon Code" name="code" tabindex="0"
                    value="" aria-required="true" required="" wire:model="code">
            ...
            <fieldset class="name">
                ...
                <input class="flex-grow" type="text" placeholder="Coupon Value" name="value" tabindex="0"
                    value="" aria-required="true" required="" wire:model="value">
                @error('value')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </fieldset>
            <fieldset class="name">
                ...
                <input class="flex-grow" type="text" placeholder="Cart Value" name="cart_value" tabindex="0"
                    value="" aria-required="true" required="" wire:model="cart_value">
                @error('cart_value')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </fieldset>
            <fieldset class="name">
                ...
                <input class="flex-grow" type="date" placeholder="Expiry Date" name="expiry_date" tabindex="0"
                    value="" aria-required="true" required="" wire:model="expiry_date">
                @error('expire_date')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </fieldset>
